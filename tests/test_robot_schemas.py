...
from robot_schemas import GoalDraft, SpeechReply, Transcript, JevDecisions
...
class Contracts(unittest.TestCase):
    def test_single_nonempty_reply(self):
        for reply in ('', ' ', [], None):
            with self.assertRaises(ValidationError):
                SpeechReply(text=reply)
        self.assertEqual(SpeechReply(text=' one ').text, 'one')
